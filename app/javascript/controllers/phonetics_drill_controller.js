...
export default class extends Controller {
  static targets = [
    "card", "prompt", "promptKind", "option", "feedback", "counter",
    "board", "summary", "correct", "total", "weakList",
    "exampleHanzi", "exampleZhuyin", "examplePinyin", "exampleGloss",
    "feedbackZhuyin", "feedbackPinyin"
  ]
  static values = { items: Object, weak: Object, url: String, size: Number, stage: String }
  ...
  syncToggles() {
    this.element.querySelectorAll("[data-direction-option]").forEach((node) => {
      node.dataset.active = String(node.dataset.directionOption === this.direction)
    })
  ...
  next() {
    ...
    this.promptTarget.textContent = asking ? this.current.zhuyin : this.current.pinyin
    ...
    this.optionTargets.forEach((node, index) => {
      ...
      node.querySelector("[data-label]").textContent = option ? option.value : ""
  ...
  answer(direction) {
    ...
    const chosen = this.optionTargets.find((node) => node.dataset.direction === direction)
    const right = this.optionTargets.find((node) => node.dataset.correct === "true")
    const good = chosen && chosen.dataset.correct === "true"

    if (chosen && !good) chosen.className = `${this.optionClass()} border-red-500 bg-red-500/10`
    if (right) right.className = `${this.optionClass()} border-emerald-500 bg-emerald-500/10`
  ...
  setFeedback(zhuyin, pinyin) {
    if (this.hasFeedbackZhuyinTarget) this.feedbackZhuyinTarget.textContent = zhuyin || ""
    if (this.hasFeedbackPinyinTarget) this.feedbackPinyinTarget.textContent = pinyin || ""
  }

  setExample(item = this.current) {
    const c = item
    const has = Boolean(c && c.hanzi)
    if (this.hasExampleHanziTarget) this.exampleHanziTarget.textContent = has ? c.hanzi : ""
    if (this.hasExampleZhuyinTarget) this.exampleZhuyinTarget.textContent = has ? c.hanziZhuyin || "" : ""
    if (this.hasExamplePinyinTarget) this.examplePinyinTarget.textContent = has ? c.hanziPinyin || "" : ""
    if (this.hasExampleGlossTarget) {
      this.exampleGlossTarget.textContent = has && c.hanziGloss ? `\u2014 ${c.hanziGloss}` : ""
    }
  }
  ...
  finish() {
    ...
    this.totalTarget.textContent = this.correct + Object.values(this.misses).reduce((a, b) => a + b, 0)
