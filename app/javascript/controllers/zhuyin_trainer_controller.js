...
export default class extends Controller {
  static targets = ["option", "feedback", "counter", "board", "summary", "correct", "total", "replay"]
  ...
  play() {
    ...
    const begin = () => {
      this.leadTimer = setTimeout(() => this.audio?.play().catch(() => {}), 140)
    }
  ...
  onKey(event) {
    ...
    if (!direction) return
    event.preventDefault()
    this.grade(this.optionTargets.find((node) => node.dataset.direction === direction))
  }
  ...
  grade(node) {
    ...
    const right = this.optionTargets.find((option) => option.dataset.correct === "true")

    if (!good) node.className = `${this.optionClass()} border-red-500 bg-red-500/10`
    if (right) right.className = `${this.optionClass()} border-emerald-500 bg-emerald-500/10`

    this.results.push({ symbol: this.current.symbol, correct: good, elapsed_ms: elapsed })
