...
export default class extends Controller {
  ...
  connect() {
    ...
    if (this.autoMsValue > 0)
      this.autoTimer = setTimeout(() => this.toggle(), this.autoMsValue);
  }
  ...
  render(result) {
    ...
    const overall = Math.round(
      Number(result.overall ?? this.average(syllables)) || 0,
    );
  ...
  average(syllables) {
    const values = syllables
      .filter(Boolean)
      .map((syllable) => Number(syllable.overall) || 0);
  ...
  paintScore() {
    ...
    if (this.hasAttemptsTarget) {
      this.attemptsTarget.textContent = this.labelAttemptsValue.replace(
        "%{count}",
        this.scores.length,
      );
    }
