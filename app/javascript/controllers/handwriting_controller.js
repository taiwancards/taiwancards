...
function loadData(url) {
  if (!dataPromise) {
    dataPromise = new Promise((resolve, reject) => {
      HanziLookup.init("mmah", url, (success) =>
        success ? resolve(true) : reject(new Error("data")),
      );
    });
  }
  return dataPromise;
...
export default class extends Controller {
  ...
  connect() {
    ...
    loadData(this.urlValue)
      .then(() => this.setStatus(""))
      .catch(() => this.setStatus(this.statusTarget?.dataset.failed || "!"));
  }
  ...
  resize() {
    const side =
      ...
      this.canvasTarget.getBoundingClientRect().width;
    if (!side) return;
  ...
  recognize() {
    ...
    new HanziLookup.Matcher("mmah", this.looseness()).match(
      analyzed,
      this.limitValue,
      (matches) => this.render(matches),
    );
  }
  ...
  render(matches) {
    ...
    matches.forEach((match) => {
      this.resultsTarget.appendChild(
        this.emitValue ? this.chip(match) : this.link(match),
      );
    });
  }
  ...
  chip(match) {
    ...
    chip.addEventListener("click", () =>
      this.dispatch("pick", { detail: { character: match.character } }),
    );
    return chip;
  }
