...
export default class extends Controller {
  ...
  async next() {
    ...
    this.optionTargets.forEach((node) => { node.className = this.optionClass() })
    this.play()
  }
  ...
  play() {
    ...
    const begin = () => {
      ...
      if (this.current.stop_ms > 0) {
        this.watcher = () => {
          if (this.audio && this.audio.currentTime >= this.current.stop_ms / 1000) this.stop()
        }
  ...
  onKey(event) {
    ...
    this.grade(this.optionTargets.find((node) => Number(node.dataset.tone) === tone))
  }
  ...
  grade(node) {
    ...
    const right = this.optionTargets.find((option) => Number(option.dataset.tone) === this.current.tone)
