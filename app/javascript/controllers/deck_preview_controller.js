...
export default class extends Controller {
  ...
  applyFilters() {
    ...
    const maxFreq = this.hasMaxFreqTarget
      ? Number(this.maxFreqTarget.value)
      : 0;
    const skipKnown = this.hasSkipKnownTarget && this.skipKnownTarget.checked;
    const skipInDecks =
      this.hasSkipInDecksTarget && this.skipInDecksTarget.checked;
  ...
  selectAll() {
    this.selected = new Set(
      this.candidates.slice(0, this.limitValue).map((entry) => entry.i),
    );
    this.repaintAll();
    this.sync();
  }
  ...
  paint(index) {
    ...
    for (const span of this.spans.get(index) || [])
      span.classList.toggle("tok-on", on);
    const row = this.hasListTarget
      ? this.listTarget.querySelector(`[data-row="${index}"]`)
      : null;
