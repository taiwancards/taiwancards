...
export default class extends Controller {
  static targets = ["square", "status", "result", "rating", "elapsed", "form", "hint", "skip"]
  static values = {
    labelCorrect: String,
    labelHinted: String,
    labelSkipped: String,
  ...
  complete(index, element) {
    ...
    this.report()
    if (this.done.every(Boolean)) this.settle(this.verdict(), this.labelFor(this.verdict()))
  }

  verdict() {
    if (this.hinted) return "hard"
    return this.mistakes.reduce((total, count) => total + count, 0) === 0 ? "good" : "hard"
  }
