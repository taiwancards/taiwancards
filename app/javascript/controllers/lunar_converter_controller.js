...
export default class extends Controller {
  ...
  buildMonths() {
    ...
    const match = Array.from(this.monthTarget.options).find((o) => o.value === previous)
  ...
  buildDays() {
    ...
    this.dayTarget.value = String(previous >= 1 && previous <= month.length ? previous : 1)
  }
  ...
  toGregorian() {
    ...
    let found = null
    this.monthsOf(row).forEach((m) => {
      if (m.number === parseInt(number, 10) && m.leap === (leap === "1")) found = offset
    ...
    this.gregorianOutTarget.textContent = template
      ...
      .replace("%{weekday}", date.toLocaleDateString(document.documentElement.lang || "en", {
        weekday: "long",
        timeZone: "UTC"
      }))
  }
  ...
  toLunar() {
    ...
    if (!row) {
      this.lunarOutTarget.textContent = this.lunarOutTarget.dataset.outOfRange || ""
      return
    }
    ...
    for (let i = 0; i < months.length; i += 1) {
      if (offset < months[i].length) {
        const template = this.lunarOutTarget.dataset.template || "%{month}%{day}"
