...
export default class extends Controller {
  ...
  connect() {
    this.outside = (e) => { if (!this.popoverTarget.contains(e.target) && !e.target.closest("[data-analyzer-target=word]")) this.close() }
  ...
  open(event) {
    ...
    computePosition(anchor, this.popoverTarget, {
      ...
      Object.assign(this.popoverTarget.style, { left: `${x}px`, top: `${y}px` })
    })
  }
  ...
  async add(event) {
    ...
    await fetch(this.activateUrlValue, { method: "POST", body, headers: token ? { "X-CSRF-Token": token } : {} })
  ...
  render(token) {
    ...
    const chars = (token.chars || [])
      .map(
        (c) => `<div class="grid grid-cols-[1.5rem_auto_1fr] items-baseline gap-x-2 gap-y-0.5 text-sm">
