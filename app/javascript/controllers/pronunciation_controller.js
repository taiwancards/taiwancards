...
export default class extends Controller {
    ...
    switch (error?.name) {
      ...
      case "SecurityError":
        return window.isSecureContext === false
          ? this.labelMicInsecureValue
          : this.labelMicDeniedValue;
      case "NotFoundError":
  ...
  handleResult(result) {
    ...
    const wrong = result.syllables
      .map((s, i) => (s && s.level === "green" ? -1 : i))
      .filter((i) => i >= 0);
  ...
  paintBars(container, parts) {
    const ordered = PART_ORDER.map((id) =>
    ...
    ordered.forEach((part) => {
      ...
      bar.style.height =
        part.score == null
          ? "2px"
          : `${Math.max(2, Math.round(part.score * 0.12))}px`;
  ...
  detailCard(syllable) {
    ...
    (syllable.advisories || []).forEach((a) =>
      box.appendChild(this.note(a.note)),
    );
  ...
  diagnosticsBlock(syllable) {
    ...
    summary.className =
      "cursor-pointer select-none opacity-60 hover:opacity-100";
    summary.textContent = this.labelDetailsValue || "Details";
    box.appendChild(summary);

    const pre = document.createElement("pre");
    pre.className =
      "mt-2 overflow-x-auto whitespace-pre font-mono leading-snug";
  ...
  copyButton(source) {
    ...
    button.type = "button";
    button.className =
      "shrink-0 rounded border border-border px-1.5 py-0.5 opacity-50 transition hover:opacity-100";
    button.textContent = "⧉";
  ...
  imprint(syllable) {
    ...
    const n = (v, d) => {
      ...
      const x = Number(v);
      const digits =
        d != null ? d : Math.abs(x) >= 10 ? 0 : Math.abs(x) >= 1 ? 1 : 2;
      return x
        .toFixed(digits)
        .replace(/(\.\d*?)0+$/, "$1")
        .replace(/\.$/, "")
    ...
    const axisLine =
      ...
      axes
        .map(
          (a) =>
            `${AXIS_MARK[a.id] || a.id[0].toUpperCase()}${sgn(a.z)}⟨${a.score}⟩${this.codeTag(a.code)}`,
        )
        .join(" ");
    ...
    const rows = fields
      ...
      .map(
        (f) =>
          `${FIELD_MARK[f.field] || f.field.slice(0, 3)} ${n(f.value)}|${n(f.norm)}±${n(f.sigma)} ${sgn(f.z)}`,
      );

    const lines = [this.voiceLine(voice, n), head, axisLine];
    const curve =
      syllable.contour &&
      this.sparkPair(syllable.contour.curve, syllable.contour.reference);
  ...
  sparkPair(mine, reference) {
    ...
    const draw = (v) =>
      BARS[
        Math.min(
          BARS.length - 1,
          Math.max(0, Math.round(((v - lo) / span) * (BARS.length - 1))),
        )
      ];
  ...
  detailHeader(syllable) {
    ...
    left.appendChild(
      this.span(syllable.pinyin, "pinyin text-xs text-muted-foreground"),
    );
    ...
    const score = this.span(
      syllable.overall == null ? "—" : String(syllable.overall),
      "text-xl font-bold tabular-nums",
    );
    score.style.color = LEVELS[syllable.level] || LEVELS.gray;
    right.appendChild(score);
    const aside = syllable.rejected
      ? this.labelNotRecognizedValue
      : syllable.sounded_like;
    if (aside)
      right.appendChild(
        this.span(aside, "block text-2xs text-muted-foreground"),
      );
  ...
  partsGrid(syllable) {
    ...
    const parts = (syllable.parts || [])
      .slice()
      .sort((a, b) => PART_ORDER.indexOf(a.id) - PART_ORDER.indexOf(b.id));
  ...
  partCell(part) {
    ...
    cell.className =
      "relative overflow-hidden rounded-lg border px-1.5 py-1.5 text-center";
    cell.style.borderColor = part.level === "none" ? "" : color;
    cell.title = [part.label, part.ipa && `/${part.ipa}/`, part.cue]
      .filter(Boolean)
      .join(" · ");
    ...
    inner.appendChild(
      this.span(
        this.partTitle(part),
        "block text-3xs uppercase tracking-wide text-muted-foreground",
      ),
    );
    inner.appendChild(
      this.span(part.zhuyin || "—", "block text-base font-semibold", {
        lang: "zh-TW",
      }),
    );
    ...
    if (part.score == null) {
      ...
      inner.appendChild(
        this.span(
          this.labelWeightValue.replace("%{percent}", part.weight),
          "block text-3xs text-muted-foreground",
        ),
      );
    }
  ...
  partTitle(part) {
    const sound =
      part.id === "tone"
        ? [part.zhuyin, part.pinyin]
        : [part.zhuyin, part.ipa && `/${part.ipa}/`];
  ...
  fixBlock(part) {
    ...
    head.className =
      "flex items-baseline gap-2 text-2xs uppercase tracking-wide text-muted-foreground";
    head.appendChild(this.span(this.labelFixTitleValue, ""));
    head.appendChild(
      this.span(this.partTitle(part), "font-semibold text-foreground", {
        lang: "zh-TW",
      }),
    );
    ...
    if (part.cue)
      box.appendChild(
        this.span(part.cue, "block text-xs text-muted-foreground"),
      );
    return box;
  }
  ...
  toneChart(syllable) {
    ...
    const path = (values) =>
      values
        .map(
          (v, i) =>
            `${i ? "L" : "M"}${x(i, values.length).toFixed(1)},${y(v).toFixed(1)}`,
        )
        .join(" ");
    ...
    const bandPoints = [
      ...highs.map(
        (v, i) => `${x(i, highs.length).toFixed(1)},${y(v).toFixed(1)}`,
      ),
      ...lows
        .map((v, i) => `${x(i, lows.length).toFixed(1)},${y(v).toFixed(1)}`)
        .reverse(),
    ].join(" ");
    ...
    for (let v = Math.ceil(min / step) * step; v <= max; v += step) {
      svg.appendChild(
        this.svgNode("line", {
          ...
          y2: y(v).toFixed(1),
          stroke: "currentColor",
          "stroke-opacity": v === 0 ? "0.25" : "0.08",
          "stroke-width": "1",
        }),
      );
      const tick = this.svgNode("text", {
        x: 2,
        y: (y(v) + 3).toFixed(1),
        "font-size": "8",
        fill: "currentColor",
  ...
  toneLevel(syllable) {
    return (
      (syllable.parts || []).find((p) => p.id === "tone")?.level ||
      syllable.level
    );
  }
