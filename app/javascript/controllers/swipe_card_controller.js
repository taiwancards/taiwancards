...
export default class extends Controller {
  static targets = ["card", "back", "form", "rating", "elapsed", "goodLabel", "againLabel", "easyLabel", "hardLabel"]
  ...
  onKey(event) {
    ...
    else if (event.key === "ArrowUp") { event.preventDefault(); this.fling(0, -1, "easy") }
    else if (event.key === "ArrowDown") { event.preventDefault(); this.fling(0, 1, "hard") }
    else if (event.key === " " || event.key === "Enter") { event.preventDefault(); this.reveal() }
  }
  ...
  move(event) {
    ...
    const dx = event.clientX - this.startX
    const dy = event.clientY - this.startY
    this.decide(dx, dy, this.cardTarget.offsetWidth * 0.28, this.cardTarget.offsetHeight * 0.22)
  }
  ...
  setLabel(name, opacity) {
    ...
    if (this[`has${name.charAt(0).toUpperCase()}${name.slice(1)}Target`]) this[target].style.opacity = opacity
  }
  ...
  settle() {
    this.cardTarget.style.transition = "transform 200ms cubic-bezier(0.2, 0.8, 0.4, 1.2)"
  ...
  fling(directionX, directionY, rating) {
    ...
    this.cardTarget.style.transition = "transform 240ms ease-in, opacity 240ms ease-in"
  ...
  rateAgain() { this.fling(-1, 0, "again") }
  rateHard() { this.fling(0, 1, "hard") }
  rateGood() { this.fling(1, 0, "good") }
  rateEasy() { this.fling(0, -1, "easy") }

  rate(rating) {
    if (this.submitted) return
    this.submitted = true
    this.ratingTarget.value = rating
    if (this.hasElapsedTarget) this.elapsedTarget.value = Math.round(performance.now() - this.shownAt)
