    ...
    module Templates
      ...
      def build(key, feature_rows, meta_rows, variability: nil, vot: :own)
        ...
        tpl = {
          ...
          "f1_mid" => stat(rows.map { |r| r["f1_mid"] || mid_of(r["f1"]) }),
          "f2_mid" => stat(rows.map { |r| r["f2_mid"] || mid_of(r["f2"]) }),
          "f3_mid" => stat(rows.map { |r| r["f3_mid"] || mid_of(r["f3"]) }),
          "f1_ratio" => stat(rows.map { |r| r["f1_ratio"] }),
          "f2_ratio" => stat(rows.map { |r| r["f2_ratio"] }),
          "f2_end_ratio" => stat(rows.map { |r| r["f2_end_ratio"] }),
          "f2_delta_ratio" => stat(rows.map { |r| r["f2_delta_ratio"] }),
