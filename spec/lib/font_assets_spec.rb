...
RSpec.describe FontAssets do
  ...
  it "serves fonts from the app itself when no shared origin is configured" do
    ...
    expect(described_class.kai_full_url).to(eq("/fonts/#{described_class::KAI_FULL}"))
  end

  it "sends both sites to the same origin when one is configured" do
    with_base("https://fonts.example.test")

    expect(described_class.kai_full_url).to(start_with("https://fonts.example.test/fonts/#{described_class::KAI_FULL}"))
  end

  it "stamps the url with the contents so a replaced file is not served from cache" do
    with_base("https://fonts.example.test")

    stamp = described_class.kai_full_url[/\?v=(\w+)/, 1]

    expect(stamp).to(eq(Digest::SHA256.file(described_class.directory.join(described_class::KAI_FULL)).hexdigest[0, 8]))
  end

  it "gives the same url every time so the cache entry is reused" do
    with_base("https://fonts.example.test")

    expect(described_class.kai_full_url).to(eq(described_class.kai_full_url))
  end
