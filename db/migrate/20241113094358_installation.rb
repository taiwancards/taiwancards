...
class Installation < ActiveRecord::Migration[8.1]
  def change
    ...
    create_table("lexemes", force: :cascade) do |t|
      ...
      t.virtual(
        ...
        as: <<~SQL
          COALESCE(
            CASE data->>'tocfl_level'
              WHEN 'Novice1' THEN 1 WHEN 'Novice2' THEN 2 WHEN 'A1' THEN 3 WHEN 'A2' THEN 4
              WHEN 'B1' THEN 5 WHEN 'B2' THEN 6 WHEN 'C' THEN 7
            END,
            CASE WHEN data->>'tbcl_grade' ~ '^-?[0-9]+$' THEN (data->>'tbcl_grade')::int END
          )
        SQL
          .squish
      )
      t.virtual(
        ...
        type: :integer,
        stored: true,
        as: "CASE WHEN data->>'freq_rank' ~ '^-?[0-9]+$' THEN (data->>'freq_rank')::int END"
      )
      t.virtual(
        "moe_index",
        type: :integer,
        stored: true,
        as: "CASE WHEN data->>'moe_index' ~ '^-?[0-9]+$' THEN (data->>'moe_index')::int END"
      )
      t.virtual(
        "tbcl_grade",
        type: :integer,
        stored: true,
        as: "CASE WHEN data->>'tbcl_grade' ~ '^[0-9]+$' THEN (data->>'tbcl_grade')::int END"
      )
      ...
      t.index(
        ...
        where: "(data ->> 'chengyu') = 'true'"
      )
      t.index(
        "kind, (data ->> 'chengyu_tone'), (data ->> 'chengyu_kind'), (data ->> 'tbcl_grade')",
        name: "index_lexemes_chengyu_facets",
        where: "(data ->> 'chengyu') = 'true'"
      )
      t.index("(data ->> 'radical')", name: "index_lexemes_on_radical", where: "kind = 0")
      t.index(
        "((data ->> 'radical_number')::integer)",
        name: "index_lexemes_on_radical_number",
        where: "kind = 0 AND (data ->> 'radical_number') ~ '^[0-9]+$'"
      )
      t.index(
        "((data -> 'segments'::text))",
    ...
    create_table("reading_texts", force: :cascade) do |t|
      ...
      t.index(
        ["source_url"],
        name: "index_reading_texts_on_source_url",
        unique: true,
